...
export const fetchFilms = createAsyncThunk<any>(
    ...
    async () => {
        const query: Filter<MovieFields> = {
            selectFields: ['id', 'description', 'shortDescription', 'movieLength', 'ageRating', 'name', 'rating', 'poster', 'year', 'rating'],
            page: 1,
            limit: 9,
          };
...
const filmsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(fetchFilms.fulfilled, (state, action: PayloadAction<Film[]>) => {
